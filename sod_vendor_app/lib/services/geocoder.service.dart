...
export 'package:sod_vendor/models/address.dart';
export 'package:sod_vendor/models/coordinates.dart';
...
//
  ...
  Future<List<Address>> findAddressesFromCoordinates(
    ...
    if(AppMapSettings.isUsingVietmap){
      ...
      if(AppMapSettings.useVietMapOnApp){
        ...
        String url = "https://maps.vietmap.vn/api/reverse/v3?apikey=${apiKey}&lng=${coordinates.longitude}&lat=${coordinates.latitude}";
      ...
      if(AppMapSettings.useGoogleOnApp){
        ...
        String url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=${coordinates.toString()};key=$apiKey";
  ...
  Future<List<Address>> findAddressesFromQuery(String address) async {
    ...
    if(AppMapSettings.isUsingVietmap){
      if(AppMapSettings.useVietMapOnApp){
        final apiKey = AppStrings.vietMapMapApiKey;
        String url = "https://maps.vietmap.vn/api/autocomplete/v3?apikey=${apiKey}&text=${address}&circle_center=${myLatLng}&layers=ADDRESS";
      ...
      if(AppMapSettings.useGoogleOnApp){
        ...
        String url = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$address;key=$apiKey;location=$myLatLng;region=$region";
        final result = await get(
          Api.externalRedirect,
          queryParameters: {"endpoint": url},
        );
  ...
  Future<Address> fecthPlaceDetails(Address address) async {
    ...
    if(AppMapSettings.isUsingVietmap){
      if(AppMapSettings.useVietMapOnApp){
        final apiKey = AppStrings.vietMapMapApiKey;
        String url = "https://maps.vietmap.vn/api/place/v3?apikey=${apiKey}&refid=${address.gMapPlaceId}";
        final result = await get(
          Api.externalRedirect,
          queryParameters: {"endpoint": url},
        );
      ...
      if(AppMapSettings.useGoogleOnApp){
        final apiKey = AppStrings.googleMapApiKey;
        String url = "https://maps.googleapis.com/maps/api/place/details/json?fields=address_component,formatted_address,name,geometry;place_id=${address.gMapPlaceId};key=$apiKey";
        final result = await get(
          Api.externalRedirect,
          queryParameters: {"endpoint": url},
        );
