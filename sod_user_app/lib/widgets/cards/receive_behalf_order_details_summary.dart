...
class ReceiveBehalfOrderDetailsSummary extends StatelessWidget {
  const ReceiveBehalfOrderDetailsSummary(this.order, {Key? key}) : super(key: key);
  final Order order;
  ...
  Widget build(BuildContext context) {
    ...
    return VStack(
      [
        ...
        AmountTile("Order value".tr(), (order.receiveBehalfOrder!.orderValue ?? 0).currencyValueFormat()).py2(),
        AmountTile(
          "Serive Fee".tr(),
          "+ " + "$currencySymbol ${order.receiveBehalfOrder!.serviceFee ?? 0}".currencyFormat(currencySymbol),
        ).py2(),
        AmountTile(
          "Payment Fee".tr(),
          "+ " + "$currencySymbol ${order.receiveBehalfOrder!.paymentFee ?? 0}".currencyFormat(currencySymbol),
        ).py2(),
